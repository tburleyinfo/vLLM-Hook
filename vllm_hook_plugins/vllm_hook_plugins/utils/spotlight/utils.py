...
def compute_query_preserving_spotlight_bias(
    ...
    """
    Apply a query-preserving Spotlight variant.

    This is a novel aggregation-granularity ablation, not a replacement for
    canonical Spotlight. It preserves query position in the internal controller:
    psi_current and bias_value have shape [batch, q_len], while head treatment
    remains canonical by aggregating across heads.
    ...
    Args:
        ...
        return_diagnostics: When true, also return per-sample query diagnostics.

    Returns:
        Modified attention weights, same shape as logits. If return_diagnostics
        is true, returns (weights, diagnostics).
    """
    if logits.ndim != 4:
        raise ValueError(
    ...
    batch_size, _num_heads, q_len, k_len = logits.shape
    ...
    for batch_idx, ranges in enumerate(span_ranges):
        sample_diag: Dict[str, torch.Tensor] = {}
        if not ranges:
            if return_diagnostics:
                zeros = torch.zeros(q_len, device=logits.device, dtype=torch.float32)
                sample_diag = {
                    "psi_current": zeros,
                    "bias_value": zeros,
                    "should_steer": torch.zeros(q_len, device=logits.device, dtype=torch.bool),
                    "causally_reachable": torch.zeros(q_len, device=logits.device, dtype=torch.bool),
                }
        ...
        highlighted_mass = (
            modified_weights[batch_idx] * union_mask
        ).sum(dim=(0, 2))  # [Q], summed over H,K
        total_mass = modified_weights[batch_idx].sum(dim=(0, 2)).clamp_min(eps)
        psi_current = highlighted_mass / total_mass

        # Separate causally unreachable positions (zero span mass under the
        # current mask) from tiny-but-positive values that can be steered.
        ...
        if torch.any(should_steer):
            bias_mask = (
                bias_value.view(1, q_len, 1).to(logits.dtype)
                * union_mask.to(logits.dtype)
            )  # [1, Q, K], broadcast over H
...
def generate_with_query_preserving_spotlight(
    ...
    **kwargs
    ...
    """
    Generate with the query-preserving Spotlight worker.

    Configure HookLLM with worker_name="probe_spotlight_query_preserving".
    The SamplingParams payload remains compatible with canonical Spotlight so
    notebook comparisons can differ only by worker selection.
    """
