...
DEFAULT_SPOTLIGHT_MODEL = "Qwen/Qwen2-1.5B-Instruct"
...
SPOTLIGHT_EMPH_STRINGS = ["Return the response for the following as a JSON:"]
SPOTLIGHT_ALPHA = 0.1
...
def model_for(args: argparse.Namespace) -> str:
    ...
    return {
        ...
        "spotlight": DEFAULT_SPOTLIGHT_MODEL,
    }[args.experiment]
...
def ensure_config(args: argparse.Namespace, out_dir: Path) -> Path | None:
    if args.config_file:
...
def worker_analyzer(args: argparse.Namespace) -> tuple[str, str | None]:
    ...
    if args.experiment == "spotlight":
        return "probe_spotlight", None
...
def make_llm(args: argparse.Namespace, out_dir: Path):
    ...
    args._resolved_config_file = str(config_file) if config_file is not None else ""
...
def input_signature(args: argparse.Namespace, **values: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "model": model_for(args),
        "temperature": 0.0 if args.experiment in {"steer-activation", "spotlight"} else args.temperature,
        "top_p": args.top_p,
...
def run_spotlight(args: argparse.Namespace, out_dir: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    from vllm import SamplingParams
    ...
    comparison_key = set_comparison_identity(
        ...
        input_signature(
            ...
            emph_strings=SPOTLIGHT_EMPH_STRINGS,
    ...
    baseline_text = output_text(baseline)
    spotlight_text = output_text(spotlight)
    baseline_tokens = output_tokens(baseline)
    spotlight_tokens = output_tokens(spotlight)
    metrics = {
        **comparison_fields(args),
        "temperature": 0.0,
        "spotlight_alpha": SPOTLIGHT_ALPHA,
        "text_changed_by_spotlight": spotlight_text != baseline_text,
        "tokens_changed_by_spotlight": spotlight_tokens != baseline_tokens,
    ...
    records = [
        {
            ...
            "baseline_tokens": baseline_tokens,
            "spotlight_tokens": spotlight_tokens,
            "text_changed_by_spotlight": metrics["text_changed_by_spotlight"],
            "tokens_changed_by_spotlight": metrics["tokens_changed_by_spotlight"],
        }
    ]
    reset_prefix_cache(llm)
    return metrics, records
